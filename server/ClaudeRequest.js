...
const TOKEN_REFRESH_METHOD = 'OAUTH';
...
class ClaudeRequest {
  ...
  constructor(req) {
    ...
    if (apiKey && apiKey.includes('sk-ant')) {
      ...
      ClaudeRequest.cachedAccountId = null;
      ClaudeRequest.usingApiKey = true;
    } else {
      ClaudeRequest.usingApiKey = false;
    }
  ...
  stripTtlFromCacheControl(body) {
    ...
    if (Array.isArray(body.system)) processContentArray(body.system);
    if (Array.isArray(body.messages)) {
      body.messages.forEach(message => {
        if (message && Array.isArray(message.content)) processContentArray(message.content);
      });
    }
    return body;
  }
  ...
  filterSamplingParams(body) {
    ...
    if (hasTemperature && hasTopP) {
      ...
      if (tempIsDefault && topPIsDefault) { delete body.top_p; }
      else if (topPIsDefault) { delete body.top_p; }
      else if (tempIsDefault) { delete body.temperature; }
      else { delete body.top_p; }
    } else if (hasTopP && body.top_p === 1.0) { delete body.top_p; }
    else if (hasTemperature && body.temperature === 1.0) { delete body.temperature; }
    return body;
  }

  async getAuthToken() {
    if (ClaudeRequest.usingApiKey && ClaudeRequest.cachedToken) {
      return ClaudeRequest.cachedToken;
    }
    const result = await this.loadOrRefreshToken();
    ClaudeRequest.cachedToken = 'Bearer ' + result.token;
    ClaudeRequest.cachedAccountId = result.accountId;
    return ClaudeRequest.cachedToken;
  }
  ...
  async loadOrRefreshToken() {
    try {
      if (OAuthManager.isAuthenticated()) {
        Logger.debug('Using OAuthManager tokens (multi-account)');
        return await OAuthManager.getValidAccessToken();
      }
      if (FALLBACK_TO_CLAUDE_CODE) {
        ...
        return { token: token.replace('Bearer ', ''), accountId: null };
      }
  ...
  loadCredentialsFromFile() {
    if (process.platform === 'win32') {
      ...
      if (fs.existsSync(nativePath)) return fs.readFileSync(nativePath, 'utf8');
  ...
  writeCredentialsToFile(credentialsJson) {
    if (process.platform === 'win32') {
      ...
      fs.writeFileSync(path.join(os.homedir(), '.claude', '.credentials.json'), credentialsJson, 'utf8');
    }
  }
  ...
  async refreshTokenWithOauth() {
    ...
    try { return await ClaudeRequest.refreshPromise; }
    finally { ClaudeRequest.refreshPromise = null; }
  }
  ...
  async _doRefresh() {
    try {
      const credentialsData = this.loadCredentialsFromFile();
      const credentials = JSON.parse(credentialsData);
      const refreshTokenVal = credentials.claudeAiOauth && credentials.claudeAiOauth.refreshToken;
      const refreshData = { grant_type: 'refresh_token', refresh_token: refreshTokenVal, client_id: '9d1c250a-e61b-44d9-88ed-5944d1962f5e' };
      const options = {
        hostname: 'console.anthropic.com', port: 443, path: '/v1/oauth/token', method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json, text/plain, */*', 'User-Agent': 'claude-code-proxy/1.0.0' }
      };
        ...
        const req = https.request(options, (res) => {
          ...
          res.on('end', () => {
            try {
              ...
              else reject(new Error('OAuth request failed: ' + (parsed.error || responseData)));
            } catch (e) { reject(new Error('Invalid JSON response: ' + responseData)); }
          });
        });
        req.setTimeout(10000, () => { req.destroy(); reject(new Error('OAuth request timeout')); });
        req.on('error', reject);
      ...
      this.writeCredentialsToFile(JSON.stringify(credentials));
      Logger.info('Token refreshed successfully');
      return 'Bearer ' + response.access_token;
    } catch (error) {
      if (error.code === 'ENOENT') {
        const errorMsg = process.platform === 'win32'
          ? 'Failed to load credentials: Claude credentials file not found in WSL.'
          : 'Claude credentials not found. Please ensure Claude Code is installed and you have logged in.';
        throw new Error(errorMsg);
      }
      if (error.message.includes('invalid_grant')) throw new Error('Refresh token expired. Please log in again through Claude Code');
      if (error.message.includes('timeout')) throw new Error('Token refresh timeout. Please check your internet connection');
  ...
  getHeaders(token) {
    ...
    if (this.BETA_HEADER) headers['anthropic-beta'] = this.BETA_HEADER;
    return headers;
  }

  processRequestBody(body, presetName) {
    if (!body) return body;
    const systemPrompt = { type: 'text', text: 'You are Claude Code, Anthropic\'s official CLI for Claude.' };
    if (body.system) {
      if (Array.isArray(body.system)) body.system.unshift(systemPrompt);
      else body.system = typeof body.system === 'string' ? [systemPrompt, { type: 'text', text: body.system }] : [systemPrompt, body.system];
    } else { body.system = [systemPrompt]; }
    if (presetName) this.applyPreset(body, presetName);
  ...
  loadPreset(presetName) {
    if (ClaudeRequest.presetCache.has(presetName)) return ClaudeRequest.presetCache.get(presetName);
    try {
  ...
  applyPreset(body, presetName) {
    ...
    if (!preset) { Logger.warn('Unknown preset: ' + presetName); return; }
    if (preset.system) body.system.push({ type: 'text', text: preset.system });
    ...
    if (suffix && body.messages && body.messages.length > 0) {
      ...
      if (lastUserIndex !== -1) {
        body.messages.splice(lastUserIndex + 1, 0, { role: 'user', content: [{ type: 'text', text: suffix }] });
      }
    }
    Logger.debug('Applied preset: ' + presetName);
  }

  async makeRequest(body, presetName, token) {
    if (!token) token = await this.getAuthToken();
    ...
    const options = {
      hostname: urlParts.hostname, port: urlParts.port || 443,
      path: urlParts.pathname, method: 'POST', headers: headers
    };
  ...
  _isRateLimitOrOverload(statusCode, responseBody) {
    if (statusCode === 429 || statusCode === 529) return true;
    if (responseBody) {
      try {
        const parsed = typeof responseBody === 'string' ? JSON.parse(responseBody) : responseBody;
        if (parsed.error && parsed.error.type === 'overloaded_error') return true;
      } catch (e) { /* ignore */ }
    }
    return false;
  }

  async handleResponse(res, body, presetName) {
    try {
      ...
      if (claudeResponse.statusCode === 401) {
        ...
        try {
          ...
          Object.keys(retryResponse.headers).forEach(key => { res.setHeader(key, retryResponse.headers[key]); });
      ...
      // Handle 429/529 with token rotation (multi-account only)
      if (!ClaudeRequest.usingApiKey && ClaudeRequest.cachedAccountId &&
          (claudeResponse.statusCode === 429 || claudeResponse.statusCode === 529)) {
        const exhaustedId = ClaudeRequest.cachedAccountId;
        Logger.info('Got ' + claudeResponse.statusCode + ' on ' + exhaustedId + ', marking exhausted and rotating...');
        const errorBody = await this._readResponseBody(claudeResponse);
        OAuthManager.markExhausted(exhaustedId);
        try {
          const next = await OAuthManager.getNextToken(exhaustedId);
          Logger.info('Retrying with account ' + next.accountId);
          ClaudeRequest.cachedToken = 'Bearer ' + next.token;
          ClaudeRequest.cachedAccountId = next.accountId;
          const retryResponse = await this.makeRequest(body, presetName, 'Bearer ' + next.token);
          res.statusCode = retryResponse.statusCode;
          Object.keys(retryResponse.headers).forEach(key => { res.setHeader(key, retryResponse.headers[key]); });
          this.streamResponse(res, retryResponse);
          return;
          ...
          res.writeHead(claudeResponse.statusCode || 429, { 'Content-Type': 'application/json' });
          res.end(errorBody);
          return;
        }
      }

      // Check non-streaming error responses for overloaded_error
      if (!ClaudeRequest.usingApiKey && ClaudeRequest.cachedAccountId) {
        const contentType = claudeResponse.headers['content-type'] || '';
        if (!contentType.includes('text/event-stream') && claudeResponse.statusCode >= 400) {
          const errorBody = await this._readResponseBody(claudeResponse);
          if (this._isRateLimitOrOverload(claudeResponse.statusCode, errorBody)) {
            const exhaustedId = ClaudeRequest.cachedAccountId;
            Logger.info('Got overloaded error on ' + exhaustedId + ', rotating...');
            OAuthManager.markExhausted(exhaustedId);
            try {
              const next = await OAuthManager.getNextToken(exhaustedId);
              ClaudeRequest.cachedToken = 'Bearer ' + next.token;
  ...
  streamResponse(res, claudeResponse) {
        ...
        for (const line of lines) {
          if (line.startsWith('data: ')) {
            ...
            if (data.type === 'content_block_delta') {
              if (data.delta && data.delta.type === 'text_delta') return { text: data.delta.text };
              if (data.delta && data.delta.type === 'thinking_delta') return { thinking: data.delta.thinking };
            }
          }
        }
      } catch (e) { }
    ...
    if (contentType.includes('text/event-stream')) {
      ...
      if (Logger.getLogLevel() >= 3) {
        ...
        debugStream.on('end', () => { Logger.debug('Streaming response sent back to client'); });
      } else {
        claudeResponse.pipe(res);
        claudeResponse.on('end', () => { Logger.debug('Streaming response sent back to client'); });
      }
    } else {
      res.removeHeader('content-encoding');
      let responseData = '';
